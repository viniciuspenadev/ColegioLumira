...
<head>
    ...
    <!-- Common Assets -->
    <?php
    // A URL publica desta pagina e /vagas/{id}, nao o caminho do arquivo
    $canonical_path = '/vagas/' . rawurlencode((string) $id);
    include '../includes/meta.php';

    // JobPosting: e o que faz a vaga entrar no Google Empregos.
    // O 'CLT' do cadastro vira FULL_TIME, o vocabulario que o schema.org aceita.
    $tipo = strtoupper((string) ($job['type'] ?? ''));
    $employment = str_contains($tipo, 'ESTAG') ? 'INTERN'
        : (str_contains($tipo, 'MEIO') || str_contains($tipo, 'PARCIAL') ? 'PART_TIME' : 'FULL_TIME');

    $publicado = !empty($job['created_at']) ? date('Y-m-d', strtotime($job['created_at'])) : date('Y-m-d');
    ...
    $job_schema = [
        ...
        // Sem validThrough o Google trata a vaga como expirada depois de um tempo
        'validThrough' => date('Y-m-d', strtotime($publicado . ' +90 days')),
        'employmentType' => $employment,
        'directApply' => true,
        'hiringOrganization' => [
            '@type' => 'Organization',
            'name' => 'Colégio Lumirá',
            'sameAs' => 'https://www.colegiolumira.com',
    ...
    if (!empty($job['workload'])) {
        $job_schema['workHours'] = $job['workload'];
    }
    ?>
    <script type="application/ld+json">
<?php echo json_encode($job_schema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT); ?>
    </script>
</head>
